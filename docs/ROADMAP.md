...
| ID | 태스크 | 상태 | 비고 |
|:---:|--------|:----:|------|
| T-007 | 데모 데이터 세트 확충 | 🔲 | 건물 3개, 방 5개, 카메라 8개, 화재 이벤트 10개 |
| T-008 | API base URL 환경 변수화 | 🔲 | `.env`로 분리, 로컬/프로덕션 전환 |
| T-009 | CCTV 화면 데모 영상 | 🔲 | 로컬 번들 MP4로 영상 재생 시뮬레이션 |
| T-010 | 화재 이벤트 수동 트리거 | 🔲 | 개발자 메뉴에서 시뮬레이션 버튼. 의존: T-007 |
