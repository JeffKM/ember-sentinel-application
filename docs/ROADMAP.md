...
| 3 | 엣지 IoT 시뮬레이터 | P0 | 3 | 🔲 대기 | 0/3 |
| 4 | 백엔드 보안 강화 | P1 | 3 | 🔲 대기 | 0/3 |
| 5 | 백엔드 테스트 강화 | P1 | 4 | 🔲 대기 | 0/4 |
...
| T-012 | 설정 파일 외부화 | 🔲 | `config.yaml`로 서버 URL, 모델 경로, BLE MAC 등 분리 |
| T-013 | BLE 모킹 | 🔲 | `--no-ble` 플래그로 BLE 없이 실행 가능. 의존: T-012 |
...
| ID | 태스크 | 상태 | 비고 |
|:---:|--------|:----:|------|
| T-014 | 엣지 디바이스 API Key 인증 | 🔲 | `X-Device-API-Key` 헤더 검증, CameraEdge에 `api_key` 필드 추가 |
| T-015 | Rate Limiting 적용 | 🔲 | 화재 이벤트 API — IP + 디바이스 기반 1분 1회. 의존: T-014 |
| T-016 | JWT Refresh Token 회전 | 🔲 | Refresh Token 사용 시 새 토큰 발급 + 이전 토큰 무효화 (Redis TTL) |
...
| ID | 태스크 | 상태 | 비고 |
|:---:|--------|:----:|------|
| T-017 | 핵심 도메인 단위 테스트 | 🔲 | `FireEventCommandService`, `AuthService`, `RoomCommandService` |
| T-018 | 통합 테스트 (Testcontainers) | 🔲 | `@SpringBootTest` + Testcontainers (PostgreSQL, Redis). 의존: T-001 |
| T-019 | API 엔드포인트 테스트 | 🔲 | MockMvc로 주요 API 15개+ 성공/실패 케이스. 의존: T-017 |
| T-020 | 테스트 커버리지 리포트 | 🔲 | JaCoCo + GitHub Actions 커버리지 배지. 의존: T-017~T-019 |
