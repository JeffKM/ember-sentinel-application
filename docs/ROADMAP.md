...
> **최종 갱신일**: 2026-06-01 <!-- v4.4 T-085 비고 확장: MutationObserver 비디오 수정, 데모 데이터 정합성, LoginScreen UI -->
...
| T-085 | 웹 스크롤 차단 해제 + 화면 잘림 수정 + 데스크톱 maxWidth             |  ✅  | Expo 기본 CSS(`overflow:hidden`) 오버라이드, CCTV/영상 화면 ScrollView 래핑 + 웹 고정 높이, 데스크톱 `maxWidth:480` 중앙 정렬. **v4.3 추가**: DemoVideoFallback 웹 HTML `<video>` 직접 렌더링, App.tsx MutationObserver로 모든 `<video>` 요소에 `object-fit:contain; width:100%; height:100%` 강제 적용 (expo-av 원본 해상도 오버플로 해결). LoginScreen 중복 whiteCard 제거. 데모 데이터 rooms 5/7 카메라 화재 상태 정합성 수정. FireEventVideoScreen 컨테이너 높이 600 통일 |
