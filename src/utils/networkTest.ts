 ...
 * 기본 네트워크 연결 테스트
 */
export const testBasicNetwork = async (): Promise<boolean> => {
  try {
    ...
    console.log('❌ 기본 네트워크 연결 실패:', (error as Error).message);
    return false;
  }
};
 ...
 * 종합 네트워크 진단
 */
export const runNetworkDiagnostics = async (): Promise<NetworkDiagnosticsResult> => {
  console.log('🔍 네트워크 진단 시작...\n');

  const results: NetworkDiagnosticsResult = {
    basicNetwork: false,
